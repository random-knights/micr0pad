...
// Where the AiEDs log lives, in priority order:
//   1. AIEDS_LOG_PATH                  - explicit, wins over everything
//   2. <per-user dir>/aieds-local.jsonl - the default (lib/paths.js), the same
//                                        file hooks/aieds-local.js writes
//   3. <app>/aieds-local.jsonl          - where a checkout wrote it before the
//                                        per-user directory existed
// Nothing is created here: no log means the panel shows its empty state.
const APP_DIR = path.join(__dirname, "..");
const USER_LOG = path.join(require("./paths").dataDir(), "aieds-local.jsonl");
...
const LOG_PATH = process.env.AIEDS_LOG_PATH
  ...
  : firstExisting([USER_LOG, path.join(APP_DIR, "aieds-local.jsonl")], USER_LOG);
const STATE_DIR = path.dirname(LOG_PATH);

// Cost is a MODELED list price, never a bill - see the header of
    ...
    stream.on("end", () => {
      try {
        ...
        resolve({
          ...
          totalCarbonG,
          totalTreeMin,
          // Modeled list price, and how many rows carried a priced model.
          totalCostUsd,
          pricedRows,
...
// modeled cost, and response timing.
//
