...
//
// It writes ONE line when the server stops, to analytics-local.jsonl beside
// the app (override with MICR0PAD_ANALYTICS_PATH). Nothing is transmitted:
...
// construction.
//
// NOT MODELLED: energy in watt-hours. AIEDS v2 models energy from AI token
// counts, and there is no sourced figure for what this device draws. A
// modelled number without a source would be a guess wearing a unit, so the
...
function settings(cfg = {}, env = process.env) {
  ...
  const logPath = env.MICR0PAD_ANALYTICS_PATH
    ...
    : path.join(APP_DIR, "analytics-local.jsonl");
...
class Session {
  ...
  record() {
    ...
    return sanitize({
      ...
      energyNote: "not modelled: no sourced power figure for this device",
...
function attach(bridge, cfg, env = process.env) {
  ...
  // exit covers a normal stop and an uncaught throw. SIGINT is Ctrl+C, which
  // is how this actually ends most of the time, and it does not fire exit on
  // its own unless something calls process.exit.
