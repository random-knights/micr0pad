...
// colour matrix and the two light zones in memory, answers the JSON-RPC
...
// behaviour run under node --test with no hardware.
//
...
function open() {
  // The colour matrix: key id -> the last thread frame the host sent for it.
  ...
  // light with the same colours they had a moment earlier.
  function snapshot() {
    return {
