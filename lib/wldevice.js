...
//
// node-hid is an OPTIONAL dependency, loaded on first use. On a platform with
// no prebuilt binary and no compiler, npm skips it and the install still
// succeeds; open() then throws, the bridge records that as the device error,
// and the virtual pad runs. A require at the top of this file would take the
// whole server down instead.
function loadHid() {
  try {
    return require("node-hid");
  } catch (e) {
    throw new Error(
      "node-hid is not installed here, so no physical pad can be opened: " +
