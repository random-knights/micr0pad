...
//
// WHICH DEVICE. The bridge holds one implementation of the interface in
// lib/device.js and does not care which. It always prefers the physical pad:
// on start, and on every poll while it has no pad, it tries HID first. When
// HID is absent it runs lib/virtualdevice.js instead, so the app works with no
// hardware, and it keeps probing. When a pad appears the virtual one steps
// aside and the colour matrix is replayed onto the hardware.
//
// The reverse (a pad is unplugged mid-session and a virtual one takes over) is
// OFF by default, under cfg.virtualPad.onUnplug. A virtual pad appearing by
// itself while the owner is holding the real one is confusing, and the honest
// reading of an unplug is that the pad is gone.
const { open } = require("./wldevice");
const virtualdevice = require("./virtualdevice");
const { descriptorOf } = require("./device");
const pad = require("./pad");
const mapper = require("./mapper");
const herdrDefault = require("./herdr");
...
class MicropadBridge {
  // `deps` exists so the whole bridge can be exercised with no hardware and no
  // child processes: tests inject an opener and a roster. Nothing here changes
  // behaviour in a real run, where every default is the module it always was.
  constructor(cfg, deps = {}) {
    this.cfg = cfg;
    this.openHid = deps.openHid || open;
    this.openVirtual = deps.openVirtual || virtualdevice.open;
    this.herdr = deps.herdr || herdrDefault;
    this.dev = null;
    // True once a physical pad has been opened in this session. It is what
    // keeps a virtual pad from quietly replacing a pad that was unplugged.
    this.physicalSeen = false;
    this.timer = null;
    this.listeners = [];
    this.lastAgents = [];
    this.lastAssigned = [];
  ...
  // Wire a device of either kind to the handlers. The notification names are
  // the firmware's; the virtual pad emits the same ones.
  attach(dev) {
  ...
  // Open the PHYSICAL pad. Unchanged in what it does; it just no longer is the
  // only way to get a device.
  connect() {
    try {
      this.attach(this.openHid());
      this.physicalSeen = true;
      this.deviceError = null;
      return true;
    } catch (e) {
  ...
  // Is a virtual pad allowed right now? Enabled by default; suppressed once a
  // physical pad has been seen, unless the owner asked for the reverse.
  virtualAllowed() {
    const cfg = this.cfg.virtualPad || {};
    if (cfg.enabled === false) return false;
    return !this.physicalSeen || cfg.onUnplug === true;
  }

  // Serve the pad in memory. Returns false when a virtual pad is not allowed,
  // so the caller can leave the app device-less and honest about it.
  startVirtual() {
    if (this.dev || !this.virtualAllowed()) return false;
    this.attach(this.openVirtual());
    this.emit("notice", { text: "no pad found: serving a virtual pad" });
    return true;
  }

  // A physical pad appeared while the virtual one was live. Close the virtual
  // pad, open the real one, and replay the lights it was showing so the keys
  // are lit before the next poll rather than 2.5s later.
  async handover() {
    const virtual = this.dev;
    const shown = virtual.snapshot();
    this.dev = null;
    if (!this.connect()) {
      // Still no pad: keep the virtual one exactly as it was.
      this.dev = virtual;
      return false;
    }
    try { virtual.close(); } catch (_) {}
    try {
      if (shown.threads.length) await this.dev.call("v.oai.thstatus", shown.threads);
      if (shown.zones.keys || shown.zones.ambient) {
        await this.dev.call("v.oai.rgbcfg", { keys: shown.zones.keys, ambient: shown.zones.ambient });
      }
    } catch (e) {
      this.emit("notice", { text: "device connected but the replay failed: " + e.message });
    }
    this.emit("notice", { text: "physical pad connected: the virtual pad has stepped aside" });
    return true;
  }

  // The page pressed a key on the on-screen pad. Only a virtual pad accepts
  // this: a click must never be reported as a press of hardware that nobody
  // touched.
  press(id) {
    if (!this.dev || this.deviceKind() !== "virtual") {
      throw new Error("key presses from the page need the virtual pad");
    }
    return this.dev.press(id);
  }

  on(evt, fn) { this.listeners.push([evt, fn]); }
  emit(evt, data) { for (const [e, fn] of this.listeners) if (e === evt) fn(data); }
  descriptor() { return descriptorOf(this.dev); }
  deviceKind() { return this.descriptor().kind; }
  // deviceUp means a PHYSICAL pad is connected. A virtual pad is reported
  // through the descriptor instead, so nothing that counts hardware uptime
  // (lib/analytics.js) or tells the owner a pad is attached can be fooled by
  // the stand-in.
  deviceUp() { return this.deviceKind() === "hid"; }
  ...
  async cycleModel(dir) {
    try {
      const focused = await this.herdr.focusedAgent();
      ...
      await this.herdr.prompt(focused.pane_id, `/model ${model}`);
  ...
  async joyDirection(index) {
    try {
      ...
      if (index === JOY_N) await this.herdr.navigate(pane, "tab-prev");
      else if (index === JOY_S) await this.herdr.navigate(pane, "tab-next");
      else if (index === JOY_W) await this.herdr.navigate(pane, "pane-prev");
      else if (index === JOY_E) await this.herdr.navigate(pane, "pane-next");
    } catch (e) {
  ...
  async sendToFocused(command) {
    try {
      ...
      await this.herdr.prompt(focused.pane_id, command);
  ...
  async focusAgentForSlot(slot) {
    ...
    try {
      await this.herdr.focus(assigned.agent.paneID);
  ...
  async start() {
    if (!this.connect()) this.startVirtual();
    await this.refresh();
  ...
  async refresh() {
    // Hot-plug support: while there is no PHYSICAL pad, try to open one on
    // every poll. Opening is a cheap HID enumeration when nothing is attached.
    // A virtual pad never blocks that probe; it is what gets handed over.
    if (!this.pairing) {
      if (!this.dev) {
        ...
        if (!this.dev) this.startVirtual();
      } else if (this.deviceKind() === "virtual") {
        await this.handover();
      }
    }
    let agents = [];
    try { agents = await this.herdr.listAgents(); } catch (e) { agents = []; }
    ...
    this.emit("state", {
      ...
      assigned: assigned.map((x) => ({
        ...
        agent: x.agent ? { agent: x.agent.agent, status: this.herdr.statusOf(x.agent), focused: x.agent.focused, cwd: x.agent.cwd, paneID: x.agent.paneID } : null,
      })),
      deviceUp: this.deviceUp(),
