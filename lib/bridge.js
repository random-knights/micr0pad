...
class MicropadBridge {
  ...
  handleActionKey(index) {
    ...
    // The wide talk key (talk: [10, 11] in pad.js) is two physical switches
    // under one keycap for stability, and the firmware reports both as
    // separate ACT-pressed HID events for a single press, milliseconds
    // apart. Without this guard each press toggled talk on then immediately
    // off again, so dictation never had a window to hear anything - this is
    // the bug behind "push to talk stopped working." Collapse same-name
    // presses inside one debounce window into a single logical press.
    const now = Date.now();
