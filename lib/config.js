...
// mappings without touching code. config.json is per user and never shipped;
...
// config.json lives in the per-user directory from lib/paths.js, not beside
// the app: with npx the app directory is inside npm's cache and is replaced
// on every upgrade, which would reset slot names, actions and the pairing
// token. A checkout that already has a config.json beside the app gets it
// copied across once, on the first load (see migrateOnce in lib/paths.js).
//
...
// RK_MICROPAD_PORT and AIEDS_LOG_PATH. An explicit path is never migrated into.
const CONFIG_OVERRIDDEN = Boolean(process.env.RK_MICROPAD_CONFIG);
const CONFIG_PATH = CONFIG_OVERRIDDEN
  ? path.resolve(process.env.RK_MICROPAD_CONFIG)
  : path.join(paths.dataDir(), "config.json");
...
function defaults() {
  return {
    ...
    // which is the pre-virtual-pad behavior. `onUnplug` decides what happens
    ...
    // The shipped list is the production site only (www.rand0m.ai redirects
    // there, so it never sends its own Origin). A developer who reviews on a
    // staging host adds that origin to their own config.json by hand; the
    // browser sends each host's own name as the Origin, so every host a
    // pairing starts from has to be listed by name. An EXISTING config.json
    // keeps whatever list it already carries: this default only applies to a
    // config.json that has none.
    hostedOrigins: ["https://rand0m.ai"],
...
function load() {
  ...
  const base = defaults();
  let merged = base;
  if (!CONFIG_OVERRIDDEN) paths.migrateOnce("config.json", { toDir: path.dirname(CONFIG_PATH) });
...
function ensurePairingToken(cur) {
  ...
  paths.writePrivate(CONFIG_PATH, JSON.stringify(cur, null, 2));
}
