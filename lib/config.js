...
function defaults() {
  return {
    ...
    // stg.rand0m.ai is the custom staging host the owner reviews on. It fronts
    // the same site as abc-rand0m-ai.web.app, but the browser sends its own
    // name as the Origin, so it has to be listed by name or a pairing from the
    // staging page is refused before any code is checked. An EXISTING
    // config.json keeps whatever list it already carries: this default only
    // applies to a config.json that has none.
    hostedOrigins: [
      "https://rand0m.ai",
      "https://stg.rand0m.ai",
      "https://abc-rand0m-ai.web.app",
    ],
