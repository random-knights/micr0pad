...
// micro-manager bridge reads over its socket, and works on home + work PC.
...
function listAgents() {
  return new Promise((resolve, reject) => {
    execFile(herdrCmd(), ["agent", "list"], { timeout: 10000, windowsHide: true }, (err, stdout, stderr) => {
      if (err) {
...
// Field names verified against the installed herdr on 2026-09-07 and
// 2026-09-08. There is NO foreground_cwd key in this herdr's payload; an
// earlier version of this function read one, which left the cwd match rule
// in lib/mapper.js half dead. Match on cwd alone.
//
// agent_session carries the hosted agent's own session id when herdr knows
// it ({ kind: "id", value: "<uuid>" } for Claude Code). It is the join key
// between a herdr pane and the same session seen by another provider.
function normalizeAgent(a) {
  const session = a.agent_session && typeof a.agent_session === "object" ? a.agent_session : null;
  return {
    agent: a.agent || "agent",
    ...
    sessionId: session && session.kind === "id" && typeof session.value === "string" ? session.value : undefined,
    stateChangeSeq: Number.isFinite(a.state_change_seq) ? a.state_change_seq : undefined,
  };
}
...
module.exports = { listAgents, normalizeAgent, statusOf, herdrCmd, focusedAgent, prompt, navigate, cd, focus };
